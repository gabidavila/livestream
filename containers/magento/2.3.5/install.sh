...
if [[ $TOTAL_TABLES_STORE == "0" ]] ;  then 
  ...
  php -dmemory_limit=5G bin/magento setup:upgrade -vvv
 else
  echo "Configuring Magento"
  php -dmemory_limit=5G /var/www/html/bin/magento module:enable --all --clear-static-content -vvv
  php -dmemory_limit=5G bin/magento setup:di:compile -vvv
  php -dmemory_limit=5G /var/www/html/bin/magento setup:config:set -q -n --db-host="$DB_HOST" --db-name="$DB_NAME" --db-user=magento --db-password="$DB_PASSWORD" --enable-syslog-logging=1 -vvv
  php -dmemory_limit=5G bin/magento setup:upgrade -vvv
fi
